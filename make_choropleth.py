...
choropleth_df = pref_count.drop([tokyo_idx])

all = pref_count['month'].sum()

# choropleth_df['count'] = choropleth_df['month'].astype('int64')
choropleth_df['rate'] = choropleth_df['count'].apply(lambda x: (x/all)*100)
choropleth_df.drop(['month', 'title', 'address', 'lat', 'lon'], inplace=True, axis=1)

bins = list(choropleth_df['rate'].quantile([0, 0.25, 0.5, 0.75, 1]))
